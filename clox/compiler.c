...
typedef struct {
	...
	Token previous;
	bool hadError;
	// Set panicMode to escape from the interpreter.
	bool panicMode;
} Parser;

static ParseRule * getRule(TokenType typ);
...
static void parsePrecedence(Precedence p) {
	...
	if (prefixRule == NULL) {
		// At a minimum, each parser precedence must have a single type in the prefix.
...
static uint8_t makeConstant(Value value) {
	int off = addConstant(currentChunk(), value);
	if (off > UINT8_MAX) {
...
static void emitConstant(Value value) {
	uint8_t off = makeConstant(value);
	emitBytes(OP_CONSTANT, off);
}
